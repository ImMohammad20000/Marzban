...
def edit_command(call: types.CallbackQuery):
    ...
    if db_user.status == UserStatus.on_hold:
        ...
        mem_store.set(f"{call.message.chat.id}:expire_date", db_user.expire if db_user.expire else None)
    ...
    with GetDB() as db:
        ...
        if qr_select == "configs":
            for link in get_v2ray_links(user):
                ...
                f.seek(0)
                bot.send_photo(call.message.chat.id, photo=f, caption=f"<code>{link}</code>", parse_mode="HTML")
        else:
...
🚀 <b><a href="{user.subscription_url}">Subscription</a>:</b> <code>{user.subscription_url}</code>"""
    ...
    bot.send_message(call.message.chat.id, text, "HTML", reply_markup=BotKeyboard.show_links(username))
...
def add_user_from_template(call: types.CallbackQuery):
    ...
    msg = bot.send_message(
        call.message.chat.id, text, parse_mode="HTML", reply_markup=BotKeyboard.random_username(template_id=template.id)
    )
...
def random_username(call: types.CallbackQuery):
    ...
    if mem_store.get(f"{call.message.chat.id}:is_bulk", False) and not mem_store.get(
        ...
        msg = bot.send_message(
            call.message.chat.id, "how many do you want?", reply_markup=BotKeyboard.inline_cancel_action()
        )
        schedule_delete_message(call.message.chat.id, msg.id)
...
def add_user_from_template_username_step(message: types.Message):
    ...
    if mem_store.get(f"{message.chat.id}:is_bulk", False):
        msg = bot.send_message(
            message.chat.id, "how many do you want?", reply_markup=BotKeyboard.inline_cancel_action()
        )
        schedule_delete_message(message.chat.id, msg.id)
        return bot.register_next_step_handler(msg, add_user_bulk_number_step, username=username)
    else:
...
def add_user_status_step(call: types.CallbackQuery):
    ...
    msg = bot.send_message(call.message.chat.id, expiry_message, reply_markup=BotKeyboard.inline_cancel_action())
    schedule_delete_message(call.message.chat.id, msg.id)
    bot.register_next_step_handler(
        msg, add_user_expire_step, username=username, data_limit=data_limit, user_status=user_status
    )
...
def add_user_expire_step(message: types.Message, username: str, data_limit: int, user_status: str):
    ...
    if user_status == "onhold":
        ...
        msg = bot.send_message(message.chat.id, timeout_message, reply_markup=BotKeyboard.inline_cancel_action())
        schedule_delete_message(message.chat.id, msg.id)
...
"""
        ...
        if (username := mem_store.get(f"{call.message.chat.id}:username")) is None:
            try:
                ...
                pass
            return bot.send_message(
                call.message.chat.id, "❌ Bot reload detected. Please start over.", reply_markup=BotKeyboard.main_menu()
            )
...
<b>By :</b> <a href="tg://user?id={chat_id}">{full_name}</a>"""
        ...
        bot.edit_message_text(
            "⏳ <b>In Progress...</b>", call.message.chat.id, call.message.message_id, parse_mode="HTML"
        )
        with GetDB() as db:
...
<b>By :</b> <a href="tg://user?id={chat_id}">{full_name}</a>"""
                try:
                    bot.send_document(
                        TELEGRAM_LOGGER_CHANNEL_ID, open(file_name, "rb"), caption=text, parse_mode="HTML"
                    )
                    os.remove(file_name)
                except ApiTelegramException:
                    pass
    elif data == "add_data":
        schedule_delete_message(call.message.chat.id, bot.send_message(chat_id, "⏳ <b>In Progress...</b>", "HTML").id)
                    ...
                    try:
                        if user.expire and user.status not in [UserStatus.limited, UserStatus.expired]:
                            user = crud.update_user(
                                db, user, UserModify(expire=int((user.expire + relativedelta(days=days)).timestamp()))
                            )
                            counter += 1
                            f.write(
                                f"{user.username}\
